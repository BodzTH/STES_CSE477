...
/* I2C Base Addresses */
#define I2C0_BASE_ADDR 0x40020000
#define I2C1_BASE_ADDR 0x40021000
#define I2C2_BASE_ADDR 0x40022000
#define I2C3_BASE_ADDR 0x40023000
...
/* Dynamic Register Access Macros */
#define I2C_MSA_REG(base) (*((volatile uint32 *)(base + I2C_MSA_OFFSET)))
#define I2C_MCS_REG(base) (*((volatile uint32 *)(base + I2C_MCS_OFFSET)))
#define I2C_MDR_REG(base) (*((volatile uint32 *)(base + I2C_MDR_OFFSET)))
#define I2C_MTPR_REG(base) (*((volatile uint32 *)(base + I2C_MTPR_OFFSET)))
#define I2C_MCR_REG(base) (*((volatile uint32 *)(base + I2C_MCR_OFFSET)))

/*******************************************************************************
 *                              Private Variables  *
 *******************************************************************************/

static uint8 g_currentModule = I2C_DEFAULT_MODULE;
static uint32 g_i2cBase = I2C0_BASE_ADDR;
...
/*
 * Description: Update the global base address based on current module
 */
static void I2C_UpdateBaseAddress(void) {
  switch (g_currentModule) {
  case I2C_MODULE_0:
    g_i2cBase = I2C0_BASE_ADDR;
    break;
  case I2C_MODULE_1:
    g_i2cBase = I2C1_BASE_ADDR;
    break;
  case I2C_MODULE_2:
    g_i2cBase = I2C2_BASE_ADDR;
    break;
  case I2C_MODULE_3:
    g_i2cBase = I2C3_BASE_ADDR;
    break;
  default:
    g_i2cBase = I2C0_BASE_ADDR;
    break;
...
static I2C_Error I2C_WaitMasterIdle(void) {
  ...
  /* Wait while busy */
  while (I2C_MCS_REG(g_i2cBase) & I2C_MCS_BUSY) {
    if (--timeout == 0) {
...
static I2C_Error I2C_CheckError(void) {
  uint32 status = I2C_MCS_REG(g_i2cBase);
 ...
 */
void I2C_Init(void) { I2C_InitModule(I2C_MODULE_0, I2C_SPEED_STANDARD); }
...
void I2C_InitModule(uint8 module, uint32 speed) {
  ...
  I2C_UpdateBaseAddress();

  /* Enable I2C module clock */
  SYSCTL_RCGCI2C_REG |= (1 << module);

  /* Wait for clocks to stabilize */
  delay_us(10);

  if (module == I2C_MODULE_0) {
    /* I2C0: PB2 (SCL), PB3 (SDA) */
    SYSCTL_RCGCGPIO_REG |= 0x02; /* Enable Port B clock */
    delay_us(2);
  ...
  } else if (module == I2C_MODULE_1) {
    /* I2C1: PA6 (SCL), PA7 (SDA) */
    SYSCTL_RCGCGPIO_REG |= 0x01; /* Enable Port A clock */
    delay_us(2);

    GPIO_PORTA_AFSEL_REG |=
        0xC0; /* Enable alternate function on PA6, PA7 (Bits 6,7) */
    GPIO_PORTA_ODR_REG |= 0x80;         /* Enable open-drain on PA7 (SDA) */
    GPIO_PORTA_DEN_REG |= 0xC0;         /* Enable digital on PA6, PA7 */
    GPIO_PORTA_PCTL_REG &= ~0xFF000000; /* Clear PCTL for PA6, PA7 */
    GPIO_PORTA_PCTL_REG |= 0x33000000; /* Set PCTL for I2C function (PMC = 3) */
  }
  /* Add other modules here if needed */

  /* Initialize I2C Master */
  I2C_MCR_REG(g_i2cBase) = 0x10; /* Initialize I2C Master */
  ...
  /* Set Timer Period */
  I2C_MTPR_REG(g_i2cBase) = tpr & 0x7F; /* Set TPR (7 bits only) */
}
...
I2C_Error I2C_WriteByte(uint8 slaveAddr, uint8 data) {
  ...
  I2C_MSA_REG(g_i2cBase) = (slaveAddr << 1) & 0xFE;

  /* Put data to send */
  I2C_MDR_REG(g_i2cBase) = data;

  /* Send START + Data + STOP (single byte transfer) */
  I2C_MCS_REG(g_i2cBase) = I2C_MCS_START | I2C_MCS_RUN | I2C_MCS_STOP;
...
I2C_Error I2C_ReadByte(uint8 slaveAddr, uint8 *data) {
  ...
  I2C_MSA_REG(g_i2cBase) = ((slaveAddr << 1) & 0xFE) | 0x01;
  ...
  /* Read received data */
  *data = I2C_MDR_REG(g_i2cBase) & 0xFF;
...
I2C_Error I2C_WriteMultiple(uint8 slaveAddr, uint8 *data, uint8 length) {
  ...
  if (length == 1) {
    ...
    I2C_MCS_REG(g_i2cBase) = I2C_MCS_START | I2C_MCS_RUN;
  }
  ...
  for (i = 1; i < length; i++) {
    I2C_MDR_REG(g_i2cBase) = data[i];

    if (i == length - 1) {
      /* Last byte: Data + STOP */
      I2C_MCS_REG(g_i2cBase) = I2C_MCS_RUN | I2C_MCS_STOP;
    } else {
      /* Middle byte: Data (no STOP) */
      I2C_MCS_REG(g_i2cBase) = I2C_MCS_RUN;
    }
...
I2C_Error I2C_ReadMultiple(uint8 slaveAddr, uint8 *data, uint8 length) {
  ...
  if (length == 1) {
    ...
    data[0] = I2C_MDR_REG(g_i2cBase) & 0xFF;
  } else {
    /* First byte: START + Read + ACK */
    I2C_MCS_REG(g_i2cBase) = I2C_MCS_START | I2C_MCS_RUN | I2C_MCS_ACK;
    ...
    for (i = 1; i < length - 1; i++) {
      I2C_MCS_REG(g_i2cBase) = I2C_MCS_RUN | I2C_MCS_ACK;
      ...
      data[i] = I2C_MDR_REG(g_i2cBase) & 0xFF;
    }
    ...
    data[length - 1] = I2C_MDR_REG(g_i2cBase) & 0xFF;
  }
...
I2C_Error I2C_ReadRegister(uint8 slaveAddr, uint8 regAddr, uint8 *data) {
  ...
  I2C_MSA_REG(g_i2cBase) = (slaveAddr << 1) & 0xFE; /* Write mode */
  I2C_MDR_REG(g_i2cBase) = regAddr;
  I2C_MCS_REG(g_i2cBase) =
      I2C_MCS_START |
      I2C_MCS_RUN; /* START + Write (no STOP for repeated start) */
  ...
  I2C_MSA_REG(g_i2cBase) = ((slaveAddr << 1) & 0xFE) | 0x01; /* Read mode */
  I2C_MCS_REG(g_i2cBase) = I2C_MCS_START | I2C_MCS_RUN | I2C_MCS_STOP;
 ...
 */
I2C_Error I2C_WriteRegisterMultiple(uint8 slaveAddr, uint8 regAddr, uint8 *data,
                                    uint8 length) {
  I2C_Error error;
  uint8 i;
 ...
 */
I2C_Error I2C_ReadRegisterMultiple(uint8 slaveAddr, uint8 regAddr, uint8 *data,
                                   uint8 length) {
  I2C_Error error;
  ...
  I2C_MDR_REG(g_i2cBase) = regAddr;
  I2C_MCS_REG(g_i2cBase) =
      I2C_MCS_START | I2C_MCS_RUN; /* No STOP for repeated start */
...
boolean I2C_IsBusy(void) {
  return (I2C_MCS_REG(g_i2cBase) & I2C_MCS_BUSBSY) ? TRUE : FALSE;
}

/*
 * Description: Wait until I2C is ready
 */
I2C_Error I2C_WaitUntilReady(void) { return I2C_WaitMasterIdle(); }
...
boolean I2C_ScanDevice(uint8 slaveAddr) {
  ...
  if (I2C_MCS_REG(g_i2cBase) & I2C_MCS_ERROR) {
    return FALSE;
  }
